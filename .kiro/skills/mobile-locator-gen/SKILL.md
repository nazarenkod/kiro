...
## Inspect the structure (page source as JSON)
Turn a page source XML into JSON:

- Compact, readable view (recommended — only meaningful elements + key attributes):
  `node .kiro/skills/mobile-locator-gen/scripts/source-to-json.mjs --in source.xml --compact`
- Full tree (`tagName`/`attributes`/`path`/`children`):
  `node .kiro/skills/mobile-locator-gen/scripts/source-to-json.mjs --in source.xml`
  Add `--depth 4` to prune deep nesting (children replaced by a count).
- Works offline or piped from a live session:
  `node .kiro/skills/appium-connect/scripts/page-source.mjs | node .kiro/skills/mobile-locator-gen/scripts/source-to-json.mjs --compact`
