// Lifted from appium-mcp: src/locators/locator-generation.ts (Apache-2.0)
// Adapted to plain Node ESM (types removed, logger/utility imports rewired). See NOTICE.
import * as XPath from 'xpath';
const xpathSelect = XPath.select;

import log from '../utils/log.mjs';
import { isEmpty, omitNilValues } from '../utils/collection.mjs';
import {
  childNodesOf,
  domToXML,
  findDOMNodeByPath,
  ...
  try {
    ...
    for (const attrName of CHECKED_CLASS_CHAIN_ATTRIBUTES) {
      ...
      try {
        ...
        othersWithAttr = Array.isArray(result) ? result : [];
      } catch {
        continue;
      }
